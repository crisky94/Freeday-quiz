...
export default function RankingModal({ ranking, onSend, onCancel }) { 
  const [isOpen, setIsOpen] = useState(false);
      ...
      <div className='fixed inset-0 bg-gray-700 bg-opacity-50 flex justify-center items-center'>
        <div className='bg-white m-5 p-5 rounded shadow-lg max-w-lg w-full'>
          ...
          <div className='mt-4 flex justify-center text-black gap-10'>
            ...
            <button onClick={onCancel} className='hoverGradiant bg-custom-linear px-5 text-sm h-12 rounded-lg hover:transition duration-200 font-bold'>Cancelar</button>
          </div>
        </div>
      </div>
    )}
